...
describe('Rselect', () => {
  ...
  it(`should add className "hidden" to placeholder when the property
    ...
    const component = renderComponent({ value: '1' });
    const placeholder = component.find(`.${styles.placeholder}`);
  ...
  it(`should add class hidden to placeholder and remove from
       value when value didn't transmittedinto properties`, () => {
    const component = renderComponent({ options: OPTIONS });
    const option = component.find(`.${styles.option}`).at(0);

    option.simulate('click');
    expect(component.find(`.${styles.placeholder}`)
      .hasClass(styles.hidden)).to.be.equal(true);
    expect(component.find(`.${styles.value}`)
      .hasClass(styles.hidden)).to.be.equal(false);
  });
  ...
  it('should render component with autocomplete', () => {
    ...
    const placeholder = component.find(CLASS.placeholder);

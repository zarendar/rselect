...
describe('Rselect', () => {
  ...
  it('should render component by default', () => {
    ...
    expect(tags.hasClass(styles.hidden)).to.be.equal(true);
  ...
  it('should render with multi property and not empty data', () => {
    ...
    expect(tag.text()).to.be.equal(FIRST_OPTION.name);
  });

  it('should change "isFocused" state when tags was clicked', () => {
    const component = renderComponent({
      multi: true,
      options: OPTIONS
    });

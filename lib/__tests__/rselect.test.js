...
describe('Rselect', () => {
  ...
  it('should call "onChange" when component was changed', () => {
    ...
    const [[arg]] = onChange.args;
    expect(onChange).to.have.property('callCount', 1);
    expect(arg).to.be.equal(FIRST_OPTION.id);
  });

  it.only('should call "onChange" when component was changed with multi', () => {
    const onChange = sinon.spy();
    const component = renderComponent({
      multi: true,
      options: OPTIONS,
    ...
    const [[arg]] = onChange.args;
    expect(onChange).to.have.property('callCount', 1);
    expect(arg).to.deep.equal([FIRST_OPTION.id]);
  });
});
